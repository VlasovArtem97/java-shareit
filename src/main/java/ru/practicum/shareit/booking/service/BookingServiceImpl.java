...
public class BookingServiceImpl implements BookingService {
    ...
    public List<BookingDto> findBookingOwner(Long userId, String state) {
        ...
        userService.findUserById(userId);
        LocalDateTime localDateTime = LocalDateTime.now();
        itemService.getItemByUserId(userId);
        List<Booking> bookings = switch (state.toLowerCase()) {
            case "all" -> bookingRepository.findBookingOwnerItemWithStateAll(userId);
            case "current" ->
                    bookingRepository.findBookingOwnerItemWithStateCurrent(localDateTime, Status.APPROVED, userId);
            case "past" -> bookingRepository.findBookingOwnerItemWithStatePast(localDateTime, Status.APPROVED, userId);
            case "future" ->
                    bookingRepository.findBookingOwnerItemWithStateFutureAndWaiting(localDateTime, Status.APPROVED,
                            userId);
            case "waiting" ->
    ...
    public Map<Long, Booking> findAllBookingByItemIds(List<Long> itemIds, LocalDateTime now) {
        Map<Long, List<Booking>> bookingsByItemId = bookingRepository.findAllByItemIds(itemIds, now).stream()
                .collect(Collectors.groupingBy(b -> b.getItem().getId()));
        Map<Long, Booking> bookingsItem = new HashMap<>();
        for (Map.Entry<Long, List<Booking>> booking : bookingsByItemId.entrySet()) {
            Booking last = booking.getValue().stream()
                    .filter(b -> b.getEnd().isBefore(now))
                    .max(Comparator.comparing(Booking::getEnd))
                    .orElse(null);
            Booking next = booking.getValue().stream()
                    .filter(b -> b.getStart().isAfter(now))
                    .min(Comparator.comparing(Booking::getStart))
                    .orElse(null);

            bookingsItem.put(booking.getKey(),
                    Booking.builder()
                            .start(next != null ? next.getStart() : null)
                            .end(last != null ? last.getEnd() : null)
                            .build());
        }
        return bookingsItem;
    }
